...
function validate()
{
  // Préparation du contenu du payload (données qui seront envoyées vers le back-end)
  $email = $_POST['check-email'];
  $key1 = $_POST['check-key1'];
  $key2 = $_POST['check-key2'];
  $key3 = $_POST['check-key3'];

  /* Utilisation "ou copie" du modèle (entité ou classe)
  ...
  else if ($search->key1 !== $key1) // Si la clé 1 saisie est différente de celle enregistrée en base de données
  {
    echo '<h1>La clé 1 est incorrecte !</h1>';
    die();
  }
  else if ($search->key2 !== $key2) // Si la clé 2 saisie est différente de celle enregistrée en base de données
  {
    echo '<h1>La clé 2 est incorrecte !</h1>';
    die();
  }
  else if ($search->key3 !== $key3) // Si la clé 3 saisie est différente de celle enregistrée en base de données
  {
  ...
  {
  /* Alors on récupére le contenu du coffre ($content qui est l'attribut de la classe Information)
